...
#define H 601
	...
	RemplirRectangle((L-300)/2,60,300,60);
...
void AfficherTemps(int sec){
	char temps[500];
	ChoisirEcran(1);
	CopierZone(2,1,0,0,L,H,0,0);
	if (sec%60 < 10)
		sprintf(temps, "temps : %d:0%d", sec/60, sec%60);
	else
		sprintf(temps, "temps : %d:%d", sec/60, sec%60);
	EcrireTexte(0,100,temps,1);
	CopierZone(1,0,0,0,L,H,0,0);
}
...
#define NBCARTES 33 //nombre de cartes dans les dossiers bibliothèque d images
...
sprintf(IMAGE_CARTES_VERSO,"./CartesJeuxVerso/%d/symboleberserk.jpg",LH_CARTE);
...
int Click[DIM][DIM];
int Image[DIM][DIM];
char TabFichiersImage[DIM*DIM+1][100];
...
for(i=1; i<=(DIM*DIM) ; i++){
	...
	printf("Remplissage intermédiaire Indices TabNumImages(%d)= %d\n",i,TabNumImages[i]);
}
printf("\n");
...
for(i=1; i<=(DIM*DIM) ; i++){
	...
	printf("Remplissage TabNumImages(%d)= %d\n",i,TabNumImages[i]);
	printf("Remplissage TabNumImagesUniques(%d)= %d\n",TabNumImages[i],TabNumImages[i]);
	printf("Remplissage TabFichiersImage(%d)= %s\n",i,TabFichiersImage[i]);
}
printf("\n");
...
/*
		printf("Initialisation Image(%d ; %d) = %d\n",ligne,colonne,Image[ligne][colonne]);
		printf("Initialisation TabNumImages(%d) = %d\n",NumeroImage,TabNumImages[NumeroImage]);
		printf("Initialisation Image(%d ; %d) = %s\n",ligne,colonne,Image[ligne][colonne]);
*/
	}
}
printf("\n");
...
/*
	ChoisirEcran(2);
	CycleTimer = Microsecondes()+Seconde;
	while (1){
		if (Microsecondes() > CycleTimer){
			AfficherTemps(temps);
			temps += 1;
			CycleTimer = Microsecondes() + Seconde;
		}
