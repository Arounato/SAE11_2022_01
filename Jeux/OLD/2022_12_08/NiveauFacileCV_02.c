...
#define IMAGE_FOND_ECRAN "./fondjeux.jpg"//AJOUT ************
#define CarteVerso "./PhotoCarteDeJeuxVerso"
#define IMAGE_CARTE_VERSO "./PhotoCarteDeJeuxVerso/136/symboleberserk.jpg"//AJOUT ************
#define CarteRecto "./PhotoCarteDeJeuxRecto/cartes136"//MODIF ************
...
for(ligne=0;ligne<DIM;ligne++){
	for(colonne=0;colonne<DIM;colonne++){
		...
		do {
			...
			TabNumImagesUniques[NumeroImage] = NumeroRandomImage;
		} while (NumeroRandomImage==0 || imageDejaPrise2fois(TabNumImagesUniques,NumeroImage,NumeroRandomImage)==1);
	}
}
			...
			for(colonne=0;colonne<DIM;colonne++){
				...
				if(X_Clk>=x && X_Clk<=(x+LHCARTE) && Y_Clk>=y && Y_Clk<=(y+LHCARTE)){
					if(Click[ligne][colonne]==0){
						//sprintf(nomFichier,"%s/%s%d%s",CarteRecto,"carte-",Image[ligne][colonne],".jpg");
